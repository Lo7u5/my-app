...
const Messages = (props) => {
    ...
    let contactElements = props.state.contacts.map(contact => <Contact name={contact.name} id={contact.id}
    ...
    for (let i = 1; i <= contactsAmount; i++) {
        let messagesFilter = props.state.messages.filter((message) => message.dialogId === i);
        messagesById.push({
            id: i,
